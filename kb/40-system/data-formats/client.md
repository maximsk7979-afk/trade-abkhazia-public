...
| `bananaFixedPrice` | number / string | нет | **Договорная фикс-цена банана** ₽/ящик. Если задана (>0) — перебивает сетку объёма (`volume-tier`) для этого клиента и **не добавляет** надбавку за доставку (`source: "fixed"`, см. [ADR-020](../../90-decisions/ADR-020-order-intake-banana-pricing.md)). **Нельзя ниже якоря**; «устаревшая» < якоря считается по якорю (пол). Пусто/отсутствует = цена по сетке объёма. Читается в `order-service.clientFixedPrice`. Задаётся при заводе (Ева спрашивает «прайс-лист или фикс»), через `set_client_banana_price` (менеджер/ГД, в любой момент) или в карточке trade_app. Введено 2026-06-08 |
| `bananaFixedAnchor` | number / string | нет | **Якорь на момент назначения** `bananaFixedPrice`. Пишется вместе с фикс-ценой (`set_client_banana_price`/`create_client`). Нужен **каскаду переоценки**: после смены якоря видно, какие фикс-цены устарели. Пусто = клиент по сетке. Введено 2026-06-10 |
