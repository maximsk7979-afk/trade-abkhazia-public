...
- **Поток:** группа (напр. Помидоры) → открытые весовые рейсы проекта (`/trips?kind=weighted`) → выбор позиции (`/weighted-skus`, показывает товар · упаковку · нетто · ₽/кг) → кол-во упаковок → live-котировка (`/price?kind=weighted`) с пометкой «ориентировочно» → оформить/изменить/отменить.
- **Цена/кг** задаёт владелец напрямую по SKU (решение 2026-06-06; **не** из себестоимости — она неизвестна до закупки рейса, и **не** из прайс-уровня). Хранилище — KV `trade-banana-pricing-v1`-аналог `trade-weighted-pricing-v1` `{ prices:{skuId:₽/кг}, deliverySurchargePerKg }`. Владелец управляет через Еву (инструмент `manage_weighted_pricing`). SKU без цены — **не заказной**. Доставка — общая надбавка/кг (опц.). Фикс-цена/кг клиента (`client.weightedFixedPrices[skuId]`) перебивает (хук есть, UI — позже).
- **Рейс** — та же модель ЗР (`miniOrder.kind="weighted"`, замораживается `estTotal`/`unitPrice`=₽/кг/`estKg`), но **без лимита вместимости** (лимит в кг — GAP-035). Открыт = не-банановый рейс с `orderCutoffAt` в будущем.
- **Ограничение MVP:** одна заявка на клиента на рейс = одна позиция. Многопозиционная корзина (помидоры + огурцы в один рейс) — GAP-035.
