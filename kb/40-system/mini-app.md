...
## Модуль закупщика — вкладка «🚛 Закупка» (2026-06-11)

**Кто видит:** роли `buyer` и `owner` (по `req.mini.roles`; `/me` отдаёт `roles`).
**ГД (`gen_director`) — НЕ видит**: в данных закупочные цены (решение 2026-06-09).
Закупщик не видит поставщиков «Местных овощей» (PRJ-002, `isLocalSupplier`).

**Экраны:** Погрузка (рейсы «Планируется»/«В закупке» → поставщик → позиции: тарные
факт+цена ₾/ящ, весовые ящики+брутто+цена ₾/кг; план из заявок и последние цены
поставщика — справочно; live-сумма; «Завершить закупку») и Поставщики (балансы в валюте
поставщика, леджер, «Оплата»/«Возврат за брак» — ручные операции settlements, без привязки
к рейсам).

**Эндпоинты** (`requireBuyer`): `GET /buyer/trips`, `GET /buyer/trip-plan?tripId&supId`,
`POST /buyer/load` (заменяет позиции поставщика в `trip.items`; первая погрузка →
«В закупке»+каскад заявок), `POST /buyer/finish-purchase` (сводка+вопрос Евы),
`GET /buyer/suppliers`, `GET /buyer/supplier?supId`, `POST /buyer/supplier-op`
(payment | defect_refund). Логика — `buyer-service.js` (17 тестов). Уведомления о каждой
операции — исполнителю и владельцу. Завершение рейса — через чат с Евой (`complete_trip`).
...
- **Цена в каталоге:** банан — `volume-tier` (по сумме банановых ящиков, ADR-020), заказ в **ящиках**; весовые (`tp=0`) — **₽/кг** (владелец задаёт по SKU, KV `trade-weighted-pricing-v1`, инструмент Евы `manage_weighted_pricing`), заказ в **упаковках**, теор.вес = `qty×(gw−tw)`, сумма **ориентировочная** (точная — после взвешивания при отгрузке, [GAP-038](../00-meta/gaps.md#gap-038) closed: форма отгрузки trade_app взвешивает весовые у клиента и считает `shipmentTotal` по **замороженной** ₽/кг → мини-апп показывает точную сумму; см. [процесс отгрузки](../30-roles/to-be/expeditor/processes/shipment.md)). Фикс-цена клиента: банан `client.bananaFixedPrice`, весовой `client.weightedFixedPrices[skuId]` (хук; UI — [GAP-039](../00-meta/gaps.md#gap-039)). Тарные не-банановые (`tp=1`) — без механизма цены, в каталог не попадают ([GAP-040](../00-meta/gaps.md#gap-040)).
...
- **Дата заявки = дата ДОСТАВКИ** (`sale.date = trip.arrDate` — выбранный клиентом день); дата оформления — в `miniOrder.placedAt`. В trade_app «Продажи» для `order-trip`: поле **«ДОСТАВКА»** (= `sale.date`) + **«ОФОРМЛЕНО»** (= `placedAt`).
