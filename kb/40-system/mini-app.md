...
| `GET /trips` | открытые банановые рейсы + остаток мест |
| `GET /price` | цена банана по объёму/фикс (+апсейл) |
| `POST /order` | создать/изменить заявку ЗР (атомарно по лимиту, заморозка цены) |
...
- **🛒 Заказ** — меню **групп** (из `/catalog`, группа видна если есть активный SKU): тап «Бананы» → поток заказа; прочие — «скоро».
